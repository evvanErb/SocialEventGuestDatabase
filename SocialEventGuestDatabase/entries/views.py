...
from .functions.facialRecognition import *
    ...
    if request.method == 'POST':
        ...
        if guest.is_valid():
            ...
            if(Guest.objects.filter(firstName=firstNameInput, lastName=lastNameInput).exists()):
                ...
                imageToEncode = face_recognition.load_image_file('media/pics/'+request.FILES['photo'].name)
                faceEncoding = addFace(imageToEncode)
                faceEncoding = np.array2string(faceEncoding)

                #Create and save new guest
                newGuest = Guest.objects.create(firstName = firstNameInput,
                    lastName = lastNameInput, blackListed = blackListedInput,
                    photo = photoInput, encoding = faceEncoding,
                    friendsWith = friendsWithInput,
                    ageWhenEntered = ageWhenEnteredInput, notes = notesInput,
                    dateEntered = datetime.datetime.now(tz=timezone.utc))

                newGuest.save()

            #Return HTML guest created page with string of
            #whether or not guest already existed
			#or was created
